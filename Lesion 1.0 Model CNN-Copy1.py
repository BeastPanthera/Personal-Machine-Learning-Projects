...
#PART 3: The Convolution Neural Network Modelling

#1st Model:
#num_classes = 7
SIZE = 32

#AutoKeras Model Implementation
model1 = Sequential()
...
plt.title('CNN Training & Validation Loss vs. {} Epochs'.format(len(epochs)))
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.show()
...
plt.title('CNN Training & Validation Accuracy vs {} Epochs'.format(len(epochs)))
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.show()
...
plt.title('CNN Incorrect Predictions vs Labels @ {} Epochs'.format(len(epochs)))
...
plt.title('CNN Confusion Matrix @ {} Epochs'.format(len(epochs)))
...
plt.title("CNN ROC & AUC Curve @ {} Epochs".format(len(epochs)))
